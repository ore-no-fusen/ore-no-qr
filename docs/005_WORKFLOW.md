...
2. **バージョンの採番と自動Push (npm version)**
   リリースを行う際、ターミナルで以下のコマンドを実行するだけです。
   ```bash
   ...
   npm version minor
   ```
   ※ `package.json` の設定（`postversion`）により、バージョンの引き上げ、Gitタグの作成、そして **本番（GitHub）への送信（Push）までがすべて全自動** で実行されます。ユーザーが手動でPushする必要はありません。

3. **アプリ画面への表示確認**
   `vite.config.ts` で `package.json` のバージョン番号を読み取り、画面の一番下（フッターのクレジットの下）に自動で表示されます。`npm version` 実行後は、ローカルや本番環境でこの表示が変わっていることを確認します。

4. **リリースノートの作成 (GitHub Releases)**
